    ...
    async def test_includes_required_fields(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            if is_implemented:
                ...
                assert isinstance(
                    data, dict
                ), "Should return a JSON object even if not implemented"
    ...
    async def test_returns_templates(self):
        ...
        if is_implemented:
            ...
            for template in templates:
                ...
                assert any(
                    key in template for key in ["filename", "name", "type", "id"]
                ), "Templates should have an identifier"
        else:
            # Starter code - just verify it's structured correctly
            assert isinstance(
                templates, dict
            ), "Should return structured error for starter code"
